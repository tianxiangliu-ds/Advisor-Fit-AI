...
**做**：本地 CV 解析、学生事实人工审核、手动导师资料、手动论文证据、主页解析、可选万方检索候选（中文/英文）+ 作者消歧、分维度匹配、无 LLM 模板邮件、可选 LLM 润色、Markdown/JSON 导出、完整删除。
...
2. **作者消歧**：LLM 逐篇判断候选论文是否属于该导师本人（依据机构、合作作者、研究主题），疑似同名作者的论文被折叠到「⚠ 疑似同名」区，可手动加回。
3. **确认门控**：当 Agent 发现机构明显不符等歧义时，会暂停并请求人工确认后再继续。
4. **多轮重试**：结果太少时 LLM 会去掉学校重试或切换中/英文库。

无 LLM 时降级为「确定性检索 + 机构规则消歧」。检索方式可选「自动（推荐）/ 仅中文 / 仅英文」。
