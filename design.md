...
| `.brief-section-title` | 简报无锚点小标题 | 上边线 `var(--line)` / `padding-top:18px` / 序号使用 `var(--gold)`；不用 Markdown heading，避免出现 `#01` 无效链接 |

匹配简报采用**单栏正文 + 文末折叠证据**。结论旁直接解释颜色含义与证据数量；
完整证据和导师画像放在正文之后，不使用会随论文数量无限增高的右侧栏。
...
### 4.10 研究过程与技术轨迹（`.research-*` / `.trace-*`）

用途：先用普通用户能理解的语言说明**查了什么、核对到哪一步、是否需要人工确认**；
工具名、耗时、token 和 JSON 记录收进「技术详情」。这是本项目"Agent 可见"的核心展示区，
放在「论文核验」页检索控件下方，但不要求用户先理解 Harness 与工具权限。

| 元素 | 关键值 |
|---|---|
| `.research-progress` | 自动换行的阶段卡网格；桌面端最多 4 列，窄屏单列 |
| `.research-stage` | 底 `#fffdf9` / `1px solid var(--line)` / 圆角 `6px` / `padding:12px 14px` |
| `.research-stage.done` / `.current` | 已完成用 `#54806f`，当前阶段用 `#605483`；只使用既有状态色 |
...
- 概览行只出现在折叠的技术详情中，最多 5 项；超出就换行，不缩写数字。
- 任务行显示这次运行的目标（RunTrace.task），让「调了哪些工具」有上下文。
- 一次运行的轨迹区块**只在跑过 Agent 之后出现**；没跑过时不显示空面板。
- 主界面不得直接展示 `search_by_author` 等内部工具名；必须翻译成用户动作。

### 4.11 学生事实摘要（`.fact-*`）

- 默认展示使用两列事实分组卡片，而不是一行一条的长表格；每张卡片使用 `#fffdf9`、`1px solid var(--line)`、圆角 `6px`、无阴影。
- 分组标题使用 `.78rem` Arial 小标签，内容使用可换行的 `.fact-chip`；芯片只用既有浅紫 `#eeeaf5` 和正文色，不引入新状态色。
- 单一分组内容超过 `190px` 时由 `.fact-scroll` 在卡片内部滚动，页面整体不因技能过多无限加长。
- 编辑区使用 expander，字段类型必须为中文；保留的非空事实即为用户确认的事实，删除后不参与匹配、报告或邮件。

### 4.11 研究档案与联系碰撞提醒

- 历史记录使用三列卡片网格（窄屏由 Streamlit 自然折行），每页最多 9 条；
  卡片必须同时显示导师、学校与学院，不再使用无限增长的单列按钮。
- 卡片上方提供文字搜索、学校筛选和学院筛选；只展示已完成、当前访客有权查看的记录。
- 「横向比较」与「完整记录」合并为一个比较区：选中 2–5 位后才出现对比表和联系提醒。
- 联系提醒只能引用已保存的学院与论文作者数据：
  直接合著/共同论文为 `🔴`，同学院但未发现合著为 `🟠`，
  不同学院且暂未发现合作为 `🟢`，数据不足为 `⚪`。
- `🟢` 只表示「当前公开证据暂未发现明显碰撞」，不得声称同时联系一定安全。
